...
def jupyterify_code(code):
    ...
    if isinstance(last_statement, ast.Expr) and not (
            isinstance(last_statement.value, ast.Call) and isinstance(last_statement.value.func,
                                                                      ast.Name) and last_statement.value.func.id == 'print'):
        tree.body[-1] = ast.Expr(
            ast.Call(func=ast.Name(id='print', ctx=ast.Load()), args=[last_statement.value], keywords=[]))
